...
class AuthorizeRequestValidator : IAuthorizeRequestValidator
{
    readonly IdentityServerOptions _options;
    readonly IClientStore _clients;
    readonly ICustomAuthorizeRequestValidator _customValidator;
    readonly IRedirectUriValidator _uriValidator;
    readonly IResourceValidator _resourceValidator;
    readonly IUserSession _userSession;
    readonly JwtRequestValidator _jwtRequestValidator;
    readonly IJwtRequestUriHttpClient _jwtRequestUriHttpClient;
    readonly ILogger _logger;

    readonly ResponseTypeEqualityComparer _responseTypeEqualityComparer = new();
    ...
    public async Task<Either<ErrorInfo, AuthorizeRequestValidationResult>> ValidateAsync(Dictionary<string,string> parameters, Option<ClaimsPrincipal> subject) {
        _logger.LogDebug("Start authorize request protocol validation");

        var request = new ValidatedAuthorizeRequest
        {
            Options = _options,
            Subject = subject.IfNone(Principal.Anonymous),
    ...
    async Task<Either<ErrorInfo, AuthorizeRequestValidationResult>> LoadRequestObjectAsync(ValidatedAuthorizeRequest request)
    {
    ...
    async Task<AuthorizeRequestValidationResult> LoadClientAsync(ValidatedAuthorizeRequest request)
    {
        ...
        if (client.IsNone)
        {
            LogError("Unknown client or not enabled", clientId, request);
            return Invalid(request, OidcConstants.AuthorizeErrors.UnauthorizedClient, "Unknown client or client not enabled");
        }

        request.ValidatedClient = ValidatedClient.Create(client.Get());

        return Valid(request);
    }

    async Task<AuthorizeRequestValidationResult> ValidateRequestObjectAsync(ValidatedAuthorizeRequest request)
    {
        //////////////////////////////////////////////////////////
        // validate request object
        /////////////////////////////////////////////////////////
    ...
    AuthorizeRequestValidationResult ValidateCoreParameters(ValidatedAuthorizeRequest request)
    {
        //////////////////////////////////////////////////////////
        // check state
        //////////////////////////////////////////////////////////
    ...
    AuthorizeRequestValidationResult ValidatePkceParameters(ValidatedAuthorizeRequest request)
    {
    ...
    async Task<AuthorizeRequestValidationResult> ValidateScopeAsync(ValidatedAuthorizeRequest request)
    {
        //////////////////////////////////////////////////////////
        ...
        if (requirement is Constants.ScopeRequirement.Identity or Constants.ScopeRequirement.IdentityOnly && request.IsOpenIdRequest == false)
        {
        ...
        // var validatedResources = await _resourceValidator.ValidateScopesWithClient(new(request.ValidatedClient.Get().Client, request.RequestedScopes));
        //
    ...
    async Task<AuthorizeRequestValidationResult> ValidateOptionalParametersAsync(ValidatedAuthorizeRequest request)
    {
        //////////////////////////////////////////////////////////
        // check nonce
        //////////////////////////////////////////////////////////
    ...
    AuthorizeRequestValidationResult Invalid(ValidatedAuthorizeRequest request, string error = OidcConstants.AuthorizeErrors.InvalidRequest, string description = null)
    {
        return new AuthorizeRequestValidationResult(request, error, description);
    }

    AuthorizeRequestValidationResult Valid(ValidatedAuthorizeRequest request)
    {
        return new AuthorizeRequestValidationResult(request);
    }

    void LogError(string message, ValidatedAuthorizeRequest request)
    {
    ...
    void LogError(string message, string detail, ValidatedAuthorizeRequest request)
    {
        var requestDetails = new AuthorizeRequestValidationLog(request, _options.Logging.AuthorizeRequestSensitiveValuesFilter);
