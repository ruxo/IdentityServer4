...
public class DefaultPersistedGrantService : IPersistedGrantService
{
    ...
    readonly IPersistedGrantStore _store;
    readonly IPersistentGrantSerializer _serializer;
    ...
    public async Task<IEnumerable<Grant>> GetAllGrantsAsync(string subjectId)
    {
        ...
        var grants = (await _store.GetAllAsync(new(subjectId))).ToArray();
    ...
    IEnumerable<Grant> Join(IEnumerable<Grant> first, IEnumerable<Grant> second)
    {
        ...
        foreach(var other in second)
        {
            ...
            if (match != null)
            {
                match.Scopes = match.Scopes.Union(other.Scopes).Distinct().ToArray();
    ...
    public Task RemoveAllGrantsAsync(string subjectId, string? clientId = null, string? sessionId = null) =>
        _store.RemoveAllAsync(new(subjectId, ClientId: clientId, SessionId: sessionId));
}