...
public class AuthorizeInteractionResponseGenerator : IAuthorizeInteractionResponseGenerator
{
    ...
    /// <param name="consent">The consent.</param>
    /// <returns></returns>
    public virtual async Task<InteractionResponse> ProcessInteractionAsync(ValidatedAuthorizeRequest request, Option<ConsentResponse> consent)
    {
        Logger.LogTrace("ProcessInteractionAsync");

        // if (consent.IsSome && consent.Granted == false && consent.Error.HasValue && request.Subject.Get().IsAuthenticated() == false)
        // {
        // }
        //
    ...
    protected internal virtual async Task<InteractionResponse> ProcessLoginAsync(ValidatedAuthorizeRequest request) {
        var subject = request.Subject.Get();
        ...
        if (!subject.IsAuthenticated() || !isActive)
        {
            ...
            else if (!isActive) Logger.LogInformation("Showing login: User is not active");

            return new(){ IsLogin = true };
        }

        // check current idp
        var currentIdp = subject.GetIdentityProvider();
        ...
        {
            var authTime = subject.GetAuthenticationTime();
        ...
        else if (client.IdentityProviderRestrictions.Any() && !client.IdentityProviderRestrictions.Contains(currentIdp))
        {
        ...
        if (client.UserSsoLifetime.HasValue)
        {
            var authTimeEpoch = subject.GetAuthenticationTimeEpoch();
            var nowEpoch = Clock.UtcNow.ToUnixTimeSeconds();

            var diff = nowEpoch - authTimeEpoch;
            if (diff > client.UserSsoLifetime.Value)
            {
                Logger.LogInformation("Showing login: User's auth session duration: {sessionDuration} exceeds client's user SSO lifetime: {userSsoLifetime}.", diff, client.UserSsoLifetime);
                return new(){ IsLogin = true };
            }
        }
