...
public class DeviceAuthorizationResponseGenerator : IDeviceAuthorizationResponseGenerator
{
    ...
    public virtual async Task<DeviceAuthorizationResponse> ProcessAsync(DeviceAuthorizationRequestValidationResult validationResult, string baseUrl)
    {
        ...
        response.DeviceCode = await DeviceFlowCodeService.StoreDeviceAuthorizationAsync(response.UserCode,
                                                                                        new(validationResult.ValidatedRequest.Client.ClientId,
                                                                                            validationResult.ValidatedRequest.Description,
                                                                                            validationResult.ValidatedRequest.IsOpenIdRequest,
                                                                                            response.DeviceCodeLifetime,
                                                                                            Clock.UtcNow.UtcDateTime,
                                                                                            validationResult.ValidatedRequest.ValidatedResources.RawScopeValues.ToArray()));
